...
public interface MindfulnessRepository extends JpaRepository<MindfulnessLog, Long> {
    ...
    long sumDurationInMinutesByUserAndDateBetween(@Param("user") User user, @Param("start") long start, @Param("end") long end);
    
    long countByUser(User user);
    
    @Query("SELECT COALESCE(SUM(m.durationInMinutes), 0) FROM MindfulnessLog m WHERE m.user.id = :userId AND m.timestamp >= :since")
    int sumMindfulnessDurationSince(@Param("userId") Long userId, @Param("since") LocalDateTime since);
}
